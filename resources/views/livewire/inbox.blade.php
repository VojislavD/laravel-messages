...
>
    ...
    <div class="w-3/4 border border-gray-300 bg-white rounded-xl">
        <div class="w-full h-full flex items-center justify-center block" :class="selectedThread === 0 ? 'block' : 'hidden'">
            <h5 class="text-2xl text-gray-400">{{ __('Select Thread') }}</h5>
        </div>
        <div 
            class="w-full h-full flex flex-col justify-between block"
            :class="selectedThread === 1 ? 'block' : 'hidden'"
        >
            <h5 class="h-[60px] text-lg p-3.5 border-b-2 font-semibold">{{ __('To') }}: Annie Baker</h5>
